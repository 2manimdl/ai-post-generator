...
export default function TabDesign({ controls, setControls }) {
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-left-5">
        ...
        {/* OVERLAY SETTINGS (INI YANG HILANG SEBELUMNYA) */}
        <div className="space-y-3 p-4 bg-slate-900 text-white rounded-xl">
            <p className="text-[10px] font-black uppercase text-orange-400 border-b border-slate-700 pb-2">Background Overlay (Dimmer)</p>
            
            <div className="flex justify-between items-center mb-2">
                <span className="text-xs font-bold">Warna Gelap</span>
                <input 
                    type="color" 
                    value={controls.overlayColor} 
                    onChange={e => setControls({...controls, overlayColor: e.target.value})} 
                    className="w-6 h-6 border-none cursor-pointer rounded"
                />
            </div>
            
            <div>
                <div className="flex justify-between text-[9px] mb-1 text-slate-300">
                    <span>Transparan</span>
                    <span>{Math.round(controls.overlayOpacity * 100)}%</span>
                    <span>Gelap</span>
                </div>
                <input 
                    type="range" 
                    ...
                    onChange={e => setControls({...controls, overlayOpacity: e.target.value})} 
